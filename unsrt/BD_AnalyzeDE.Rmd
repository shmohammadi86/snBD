...
```{r}
df.DE = readr::read_rds("~/BD/results/datasets/allDE_PB_5conditions.rds")
names(df.DE$table) = c("BD", "BDnotSZ", "BDandSZ", "SZ", "SZnotBD")
DE.tables = df.DE$table[1:4]
...
storeTable(list(Pi1 = Pi1s), name = "Pi1", tables.path = tables.path)
...
for(i in 1:length(DE.tables)) {
  ...
  storeTable(enrichments, sprintf("SynGO_fgsea_%s", names(DE.tables)[[i]]), tables.path = tables.path)
  ...
  storeFigure(ht, sprintf("SynGO_fgsea_%s", names(DE.tables)[[i]]), extension = "pdf", figures.path = figures.path, width = 14)
}
...
suppressWarnings(ids <- AnnotationDbi::mapIds(org.Hs.eg.db, keys = rownames(all.scores), keytype = "SYMBOL", column = "ENSEMBL", multiVals = "first"))
ids[is.na(ids)] = ""

df = cbind(data.frame(Genes = ids), as.data.frame(all.scores))
df = df[ids != "", ]
df = df[match(sort(unique(df$Genes)), df$Genes), ]

readr::write_tsv(df, "~/BD/results/tables/tstats.tsv")
...
ll = lapply(dl, function(cond) {
  print(cond)
  file.name = sprintf("%s/BD/%s_UpAndDown.gsa.out", MAGMA.path, cond)
