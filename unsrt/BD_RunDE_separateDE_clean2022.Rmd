...
```{r}
cell.tbl = muscat:::.n_cells(pb.sce)
ncells = apply(cell.tbl, 2, function(x) {y = as.numeric(x); y = y / sum(y); return(y)})
rownames(ncells) = rownames(cell.tbl)
colnames(ncells) = pb.sce$ID

Ex.perc = (fast_column_sums(ncells[grepl("^Ex", rownames(ncells)) & !grepl("^Ex-NRGN", rownames(ncells)), ]))
names(Ex.perc) = pb.sce$ID
filtered.samples = which(Ex.perc < 0.2) # SZ3 SZ15 BD24 SZ24
pb.sce = pb.sce[, -filtered.samples]

y <- int_colData(pb.sce)$n_cells
...
```{r}
  ...
  for(cond in c("SZ", "BD")) {
    col.mask = (pb.sce$Phenotype == cond) | (pb.sce$Phenotype == "CON")
    
    gg = intersect(rownames(pb.sce), rownames(pb.sce.SZ)[rowData(pb.sce.SZ)$capture.rate > min.capture.rate])
    gene.mask = rownames(pb.sce) %in% gg
    pb.sce.filtered = pb.sce[gene.mask, col.mask]
    # colnames(pb.sce.filtered) = pb.sce.filtered$ID
    # pb.sce.filtered = pb.sce.filtered[, colnames(pb.sce.filtered.sz)]

    # pb.sce.filtered.sz = pb.sce.SZ[rowData(pb.sce.SZ)$capture.rate > min.capture.rate, pb.sce$ID[col.mask]]
        
    # X1 = assays(pb.sce.filtered)[["Ex-NRGN"]]
    # X2 = assays(pb.sce.filtered.sz)[["Ex-NRGN"]]
    
    
    
    # pb.sce.filtered = pb.sce.SZ
    pb.sce.filtered$Phenotype = droplevels(pb.sce.filtered$Phenotype)
    pb.sce.filtered$group_id = pb.sce.filtered$Phenotype
    ...
    contrast.mat <- makeContrasts(contrasts = sprintf("Phenotype%s-PhenotypeCON", cond), levels = design.mat)
  ...
  DFs = df.DE$table[[1]]
  # DFs = DE.old$McLean$table$PhenotypeSZ
  DFs = DFs[!(names(DFs) %in% c("Ex-NRGN", "Pericytes", "Endo"))]
  ...
  storeDataset(df.DE, sprintf("Phenotype%s_only_PB_final_Feb2022", cond), dataset.path = dataset.path)
  storeTable(DFs, name = sprintf("Phenotype%s_only_PB_final_Feb2022", cond), tables.path = tables.path)
...
```{r}
df.SZ = loadDataset(sprintf("Phenotype%s_only_PB_final_Feb2022", "SZ"), dataset.path = dataset.path)
df.BD = loadDataset(sprintf("Phenotype%s_only_PB_final_Feb2022", "BD"), dataset.path = dataset.path)

ll = lapply(df.SZ$table$`PhenotypeSZ-PhenotypeCON`, function(tbl) {v = tbl$t; names(v) = tbl$gene; return(v)})
common.genes = Reduce("intersect", lapply(ll, names))
SZ.t = do.call(cbind, lapply(ll, function(x) x[common.genes]))


ll = lapply(df.BD$table$`PhenotypeBD-PhenotypeCON`, function(tbl) {v = tbl$t; names(v) = tbl$gene; return(v)})
common.genes = Reduce("intersect", lapply(ll, names))
BD.t = do.call(cbind, lapply(ll, function(x) x[common.genes]))

common.genes = intersect(rownames(SZ.t), rownames(BD.t))
CC = cor(SZ.t[common.genes, ], BD.t[common.genes, ], method = "pearson")
...
ll = lapply(df.DE$table$PhenotypeSZ, function(tbl) {v = tbl$t; names(v) = tbl$gene; return(v)})
common.genes = Reduce("intersect", lapply(ll, names))
sz.t.new = do.call(cbind, lapply(ll, function(x) x[common.genes]))

common.genes = intersect(rownames(sz.t.old), rownames(sz.t.new))
CC = cor(sz.t.old[common.genes, ], sz.t.new[common.genes, ], method = "pearson")
