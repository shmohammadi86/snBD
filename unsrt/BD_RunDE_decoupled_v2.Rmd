...
# v[is.na(v)] = 0
# plot(density(v))
PEC.BD.Up = PEC$gene_name[(PEC$BD.fdr < 0.05) & (PEC$BD.log2FC > 0.05)]
PEC.BD.Down = PEC$gene_name[(PEC$BD.fdr < 0.05) & (PEC$BD.log2FC < -0.05)]
PEC.BD = list(BD.Up = PEC.BD.Up, BD.Down = PEC.BD.Down)


PEC.SCZ.Up = PEC$gene_name[(PEC$SCZ.fdr < 0.05) & (PEC$SCZ.log2FC > 0.05)]
PEC.SCZ.Down = PEC$gene_name[(PEC$SCZ.fdr < 0.05) & (PEC$SCZ.log2FC < -0.05)]
...
DEs.Down = lapply(DE.tables, function(DFs) {
  lapply(DFs, function(DF) {
    DF$gene[ (DF$p_adj.loc < 0.05) & (DF$logFC < -0.1)]
    # DF = DF[DF$logFC < -0, ]
    # DF$gene[order(DF$p_val)[1:500]]
  })
})
...
Z2 = (En.Down)
Z2[is.na(Z2)] = 0
Heatmap(Z1[grep("BD", rownames(Z1)), ], cluster_columns = F, column_title = "BD Up")
Heatmap(Z2[grep("BD", rownames(Z2)), ], cluster_columns = F, column_title = "BD Down")
Heatmap(Z1[grep("SZ", rownames(Z1)), ], cluster_columns = F, column_title = "SZ Up")
Heatmap(Z2[grep("SZ", rownames(Z2)), ], cluster_columns = F, column_title = "SZ Down")
...
names(DE.tables) = c("BD", "SZ", "BDnotSZ", "BDandSZ")
