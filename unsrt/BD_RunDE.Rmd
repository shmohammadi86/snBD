...
pb.sce.BD$SampleQuality = scale(pb.sce.BD$genes) #scale(log1p(pb.sce.BD$umis))
...
# formula <- ~ SampleQuality
# cd <- droplevels(data.frame(colData(pb.sce.BD)))
# design.mat <- model.matrix(formula, cd)
# cc <- colnames(design.mat)[ncol(design.mat)]
# contrast <- makeContrasts(contrasts = cc, levels = design.mat)
#         
contrast.mat <- makeContrasts(contrasts = list("SampleQuality"), levels = design.mat)


df.DE.QC = pbDS(pb.sce.BD, method = "limma-trend", min_cells = 30, design = design.mat, contrast = contrast.mat, filter = "both", treat = F)

min.capture.rate = 0.1
min.association.pval = 0.05
filtered.genes = lapply(1:length(df.DE.QC$table$SampleQuality), function(i) {
  DF = df.DE.QC$table$SampleQuality[[i]]
  is.related.to.sample.quality = DF$p_adj.loc < min.association.pval
  # is.poorly.captured = rowData(pb.sce.BD)$capture.rate[DF$gene, i] < min.capture.rate
  filter.mask = is.related.to.sample.quality
  DF$gene[filter.mask]
})
names(filtered.genes) = names(df.DE.QC$table$SampleQuality)
# cc = table(unlist(filtered.genes))
# blacklisted.genes = names(cc)[cc > sum(sapply(filtered.genes, length) > 0)/3]
# blacklisted.rows = match(blacklisted.genes, rownames(pb.sce))
...
pb.sce.BD$Age.z = scale(pb.sce.BD$Age)
pb.sce.BD$PMI.z = scale(pb.sce.BD$PMI)
# pb.sce.BD$SampleQuality = scale(pb.sce.BD$genes) #scale(log1p(pb.sce.BD$umis))
# pb.sce.BD$SampleQuality = scale(pb.sce.BD$genes) #scale(log1p(pb.sce.BD$umis))


ll = c("Young", "Old")
groups<- factor(ll[as.numeric(cut(pb.sce.BD$Age,breaks = 2))], ll)
pb.sce.BD$Age.group = groups
 
# pr.out = irlba::prcomp_irlba(as.matrix(colData(pb.sce.BD)[, 9:14]), n = 5)
# pb.sce.BD$med1 = RankNorm(pr.out$x[, 1])
# pb.sce.BD$med2 = RankNorm(pr.out$x[, 2])
# pb.sce.BD$med3 = RankNorm(pr.out$x[, 3])
# pb.sce.BD$med4 = RankNorm(pr.out$x[, 4])
# pb.sce.BD$med5 = RankNorm(pr.out$x[, 5])
...
sample.metadata = droplevels(data.frame(colData(pb.sce.BD)))
sample.metadata$Phenotype = factor(sample.metadata$Phenotype, c("CON", "BD", "SZ"))
# sample.metadata$Phenotype = factor(sample.metadata$Phenotype, c("CON", "BD"))
design.mat <- model.matrix(form, data = sample.metadata)
# colnames(design.mat)[1] = c("Intercept")

# pvals = sapply(2:ncol(design.mat), function(i) {
#   sapply(2:ncol(design.mat), function(j) cor.test(design.mat[, i], design.mat[, j], method="pearson")$p.value)
# })
# colnames(pvals) = rownames(pvals) = colnames(design.mat)[2:ncol(design.mat)]
# pvals.adj = pvals*length(pvals)
# pvals.adj[pvals.adj > 1] = 1
# En = -log10(pvals.adj)
# diag(En) = 0
...
require(RNOmni)
pb.sce.BD.scaled = pb.sce.BD
ncells = do.call(rbind, pb.sce.BD.scaled@int_colData$n_cells)
for(i in 1:length(assays(pb.sce.BD.scaled))) {
  print(i)
  X = assays(pb.sce.BD.scaled)[[i]]
  ...
  X.norm[is.na(X.norm)] = 0
  assays(pb.sce.BD.scaled)[[i]] = X.norm
}

df.DE = pbDS(pb.sce.BD.scaled, method = "limma-trend", min_cells = 30, design = design.mat, contrast = contrast.mat, filter = "none", treat = F)
